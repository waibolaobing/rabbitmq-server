...
-export([remove_default_exchange_binding_rows_of/1]).
...
add(Binding, ActingUser) ->
    add(Binding, undefined, ActingUser).

-spec add(rabbit_types:binding(), pid() | 'undefined', rabbit_types:username()) -> bind_res().
...
remove_default_exchange_binding_rows_of(Dst = #resource{}) ->
    case implicit_for_destination(Dst) of
        [Binding] ->
            rabbit_store:delete_binding(Binding);
        _ ->
...
list(VHostPath) ->
    AllBindings = rabbit_store:list_bindings(VHostPath),
    ...
    Filtered ++ implicit_bindings(VHostPath).
...
list_for_destination(#resource{virtual_host = VHost} = DstName) ->
    AllBindings = rabbit_store:list_bindings_for_destination(DstName),
    Filtered = lists:filter(fun(#binding{source = S}) ->
...
binding_checks(ConnPid) ->
    fun(Dst) ->
            check_exclusive_access(Dst, ConnPid)
    end.
